var sql = {
	CREATE : "CREATE TABLE IF NOT EXISTS cotvt_admin ("+
	...
	SELECT : "SELECT id, observer_name, observer_tel, observer_email, observation_datetime, observation_location, "+
			...
			"remarks, admin_validation, status FROM cotvt_admin where id = (?);",
	SELECTexistLIST : "SELECT * FROM cotvt_admin;",
	SELECTidINSERT : "SELECT id FROM cotvt_admin where observer_name=(?) and observer_tel=(?) and observer_email=(?) and observation_datetime=(?) and observation_location=(?) and observation_localisation=(?) and observation_region=(?) and observation_country=(?) and observation_latitude=(?) and observation_longitude=(?) and observation_number=(?) and observation_culled=(?) and counting_method_timed_swim=(?) and counting_method_distance_swim=(?) and counting_method_other=(?) and depth_range=(?) and observation_method=(?) and remarks=(?) and date_enregistrement=(?);",
	SELECTCOTLIST : "SELECT id, observation_datetime, observation_location, observation_localisation, observation_number, date_enregistrement FROM cotvt_admin where status<>'synchronized';",
	SELECTreditCOTForm : "SELECT id, observer_name, observer_tel, observer_email, observation_datetime, observation_location, observation_localisation, observation_region, observation_country, observation_latitude, observation_longitude, observation_number, observation_culled, counting_method_timed_swim, counting_method_distance_swim, counting_method_other, depth_range, observation_method, remarks FROM cotvt_admin WHERE id = (?) ;",
	INSERT : "INSERT INTO cotvt_admin(id, observer_name, observer_tel, observer_email, observation_datetime, observation_location, observation_localisation, observation_region, observation_country, observation_latitude, observation_longitude, observation_number, observation_culled, counting_method_timed_swim, counting_method_distance_swim, counting_method_other, depth_range, observation_method, remarks, status, date_enregistrement) VALUES (null, (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), 'local', (?));",
	UPDATEFORM : "UPDATE cotvt_admin set observer_name=(?), observer_tel=(?), observer_email=(?), observation_datetime=(?), observation_location=(?), observation_localisation=(?), observation_region=(?), observation_country=(?), observation_latitude=(?), observation_longitude=(?), observation_number=(?), observation_culled=(?), counting_method_timed_swim=(?), counting_method_distance_swim=(?), counting_method_other=(?), depth_range=(?), observation_method=(?), remarks=(?) where id=(?);",
	UPDATE : "UPDATE cotvt_admin set status = 'synchronized' where id = (?);",
	REMOVE : "DELETE FROM cotvt_admin where id = (?);",
	DROP: "DROP TABLE cotvt_admin;",
	DELETE : "DELETE from cotvt_admin;"
}
